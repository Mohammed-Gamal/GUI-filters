...
class FilterThread(QThread):
    ...
    def __init__(self, img, filter_index):
        super(FilterThread, self).__init__()
        self.img = img
        self.filter_index = filter_index

    def run(self):
        processed_img = f.apply_filter(self.filter_index, self.img)
...
class FiltersWindow(QMainWindow, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        self.filter_thread.finished.connect(lambda img: self.convert_cv_qt(img, self.image_2))
    ...
    def start_operation(self, filter_index):
        ...
        # Start the thread
        self.filter_thread.img = img
        self.filter_thread.filter_index = filter_index
